...
type ValidatorMgrV19TestSuite struct{}

var _ = Suite(&ValidatorMgrV19TestSuite{})

func (vts *ValidatorMgrV19TestSuite) SetUpSuite(c *C) {
	SetupConfigForTest()
}

func (vts *ValidatorMgrV19TestSuite) TestSetupValidatorNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	mgr := NewDummyMgr()
	vMgr := newValidatorMgrV19(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	vMgr1 := newValidatorMgrV19(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	vMgr2 := newValidatorMgrV19(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV19TestSuite) TestRagnarokForChaosnet(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewManagers(k)
...
func (vts *ValidatorMgrV19TestSuite) TestLowerVersion(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (vts *ValidatorMgrV19TestSuite) TestBadActors(c *C) {
	ctx, k := setupKeeperForTest(c)
	...
	nas, err := vMgr.findBadActors(ctx, constAccessor.GetInt64Value(constants.MinSlashPointsForBadValidator), constAccessor.GetInt64Value(constants.BadValidatorRedline), constAccessor.GetInt64Value(constants.RotatePerBlockHeight))
	c.Assert(err, IsNil)
	...
	c.Assert(k.SetNodeAccount(ctx, activeNode), IsNil)
	activeNode = GetRandomNodeAccount(NodeActive)
	k.SetNodeAccountSlashPoints(ctx, activeNode.NodeAddress, 150)
	c.Assert(k.SetNodeAccount(ctx, activeNode), IsNil)
	...
	k.SetNodeAccountSlashPoints(ctx, bad1.NodeAddress, 10000)
	c.Assert(k.SetNodeAccount(ctx, bad1), IsNil)
	bad2 := GetRandomNodeAccount(NodeActive)
	k.SetNodeAccountSlashPoints(ctx, bad2.NodeAddress, 10000)
...
func (vts *ValidatorMgrV19TestSuite) TestRagnarokBond(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	ver := constants.SWVersion
...
func (vts *ValidatorMgrV19TestSuite) TestGetChangedNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	ver := constants.SWVersion
...
func (vts *ValidatorMgrV19TestSuite) TestFindCounToRemove(c *C) {
	// remove one
...
func (vts *ValidatorMgrV19TestSuite) TestFindMaxAbleToLeave(c *C) {
